    ...
    <div className="min-h-screen bg-[#070b13] flex flex-col">
      ...
      <nav className="glass-panel border-b border-white/5 py-4 px-6 flex justify-between items-center shadow-lg relative z-20">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-white leading-tight">Traffic Control Authority</h1>
            <p className="text-xs text-gray-400">Green Corridor Optimization & Signal Overrides</p>
          </div>
        </div>
      ...
      {/* Main Grid */}
      <main className="flex-1 max-w-[1600px] w-full mx-auto p-6 grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch overflow-hidden">
        ...
        <div className="lg:col-span-4 flex flex-col space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="glass-panel p-4 rounded-2xl border border-emerald-500/10 flex flex-col justify-between">
              <span className="text-[10px] uppercase text-gray-400 block font-semibold">Priority Corridor Speedup</span>
            ...
            <div className="glass-panel p-4 rounded-2xl border border-emerald-500/10 flex flex-col justify-between">
              <span className="text-[10px] uppercase text-gray-400 block font-semibold">Total Time Saved Today</span>
              <div className="mt-2 flex items-baseline gap-1">
          ...
          <div className="glass-panel p-5 rounded-2xl flex-1 flex flex-col overflow-hidden">
            <h3 className="text-sm font-bold uppercase tracking-wider text-gray-300 mb-4 pb-2 border-b border-white/5 flex items-center gap-1.5">
              <ShieldAlert size={16} className="text-emerald-400" /> Active Priority Corridors
            </h3>
              ...
              {activeCorridors.length === 0 ? (
                ...
                activeCorridors.map((corr) => (
                  <div
                    ...
                    className={`p-3.5 rounded-xl border text-left cursor-pointer transition ${selectedCorridorId === corr.dispatch_id ? 'bg-emerald-600/10 border-emerald-500/40 shadow-inner' : 'bg-white/5 border-white/5 hover:border-white/10'}`}
                  >
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-[10px] uppercase font-bold text-emerald-400">Priority Overrides Engaged</span>
                      <span className="text-[9px] text-gray-500 font-semibold">{corr.vehicle_id}</span>
                    </div>
          ...
          <div className="glass-panel p-5 rounded-2xl h-[200px] flex flex-col overflow-hidden">
        ...
        <div className="lg:col-span-8 glass-panel p-4 rounded-2xl flex flex-col h-[650px]">
          <div className="mb-2 flex justify-between items-center px-2">
            <span className="text-xs font-semibold text-gray-400">Real-Time City Traffic & Corridor Simulator Map</span>
            {trackingVehicle && (
