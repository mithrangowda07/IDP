...
export default function FireStationDashboard({ user, onLogout }) {
  ...
  // Corridor states keyed by dispatchId
  ...
  const [resolveSeverity, setResolveSeverity] = useState('Medium');
  const [resolveWater, setResolveWater] = useState('');
  ...
  useEffect(() => {
    ...
    connectSocket('fire_station_user');
    ...
    socket.on('new_alert', (data) => {
      ...
      console.log('Fire station socket: new emergency alert received:', data);
      setAlerts(prev => [data.dispatch, ...prev]);
    });

    socket.on('vehicle_tracking_update', (data) => {
      console.log('Fire station socket: vehicle tracking update:', data);
    ...
    socket.on('vehicle_status_change', (data) => {
      console.log('Fire station socket: vehicle status change:', data);
    ...
    socket.on('green_corridor_update', (data) => {
      console.log('Fire station socket: corridor update:', data);
  ...
  const handleDispatch = async (alertId) => {
    if (!selectedVehicleId) {
      alert('Please select a fire engine to dispatch.');
      return;
    }
    setLoading(true);
    try {
      await serviceAPI.dispatchVehicle(user.serviceId, alertId, selectedVehicleId);
      setFeedback(`Fire Engine ${selectedVehicleId} has been dispatched!`);
  ...
  const handleSetEnRoute = async (dispatchId) => {
    try {
      ...
      setFeedback('Fire Engine is now En Route. Priority Green Corridor has been activated.');
      loadDashboardData();
      setTimeout(() => setFeedback(''), 4000);
    } catch (err) {
      console.error(err);
  ...
  const handleSetAtScene = async (dispatchId) => {
    try {
      ...
      setFeedback('Fire Engine arrived At Scene.');
      loadDashboardData();
      setTimeout(() => setFeedback(''), 4000);
    } catch (err) {
      console.error(err);
          ...
          <div className="content-grid">
            <div className="lg:col-span-5 flex flex-col gap-4 sm:gap-6 min-h-0 overflow-hidden">
                    ...
                    <div className="space-y-4">
                      <div className="bg-slate-900/50 p-4 rounded-xl border border-white/5 space-y-2 shadow-inner">
                        <div className="flex justify-between items-center">
                          <span className="text-[9px] uppercase font-bold text-red-400 tracking-wider">Incident Type</span>
                      ...
                      <div className="space-y-3 pt-3">
                        ...
                        {activeDispatch.status === 'returning' && (
                          <div className="p-3 bg-amber-500/5 border border-amber-500/20 rounded-xl text-center text-xs text-amber-400 font-bold shadow-inner">
                            Fire Engine is returning to station...
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div className="bg-amber-500/5 border border-amber-500/10 p-3.5 rounded-xl text-xs text-gray-300 shadow-inner">
                        ...
                        <div className="space-y-1.5 mt-2 text-[11px] text-gray-400">
                          ...
                          <p>Distance: <strong className="text-white">{alerts[0].distance || '1.8'} km</strong></p>
                      ...
                      <div className="space-y-3 pt-2">
                        <div>
                          <label className="block text-[9px] font-bold uppercase tracking-wider text-gray-400 mb-1.5">Select Fire Engine</label>
                          <select
                          ...
                          >
                            ...
                            {vehicles.filter(v => v.status === 'available').map(v => (
                              <option key={v.id} value={v.id}>{v.id} (Fire Engine - Available)</option>
                            ))}
                          </select>
                        </div>
                        ...
                        <button
                          ...
                          className="w-full py-3 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 transition text-xs font-bold text-white flex items-center justify-center gap-1.5 shadow-lg active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          <Send size={14} /> Dispatch Fire Engine
                        </button>
                      </div>
                    </div>
                  ) : (
              ...
              <div className="glass-panel p-5 rounded-2xl h-[220px] flex flex-col overflow-hidden shrink-0">
                <h3 className="text-xs font-bold uppercase tracking-wider text-gray-300 mb-3 pb-2 border-b border-white/5">
                  Fire Engine Vehicle Management
                </h3>
                <div className="flex-1 overflow-y-auto space-y-2 pr-1">
                  {vehicles.map((v) => (
                    <div key={v.id} className="p-2.5 rounded-xl bg-white/5 border border-white/5 flex justify-between items-center text-xs hover:border-white/10 transition">
            ...
            <div className="lg:col-span-7 map-panel-wrap">
              <div className="mb-2 flex justify-between items-center px-2 pb-1.5">
                ...
                {activeDispatch && (
                  <span className="text-[10px] text-red-400 font-bold flex items-center gap-1.5 animate-pulse">
                  ...
                  {history.map((item) => (
                    ...
                    >
                      ...
                      <div className="text-[10px] text-gray-400 space-y-1 mt-1 border-t border-white/5 pt-2">
                        ...
                        <div className="flex justify-between">
                          <span>Severity:</span>
                          <strong className="text-red-400">{item.fire_severity}</strong>
                        </div>
                        <div className="flex justify-between">
                          <span>Water Used:</span>
                          <strong className="text-blue-400">{item.water_consumption} L</strong>
                        </div>
                        <div className="flex justify-between">
              ...
              {selectedHistoryItem ? (
                <div className="flex-1 flex flex-col justify-between overflow-hidden">
                  ...
                  <div className="flex-1 overflow-y-auto space-y-5 my-4 pr-1">
                    ...
                    {/* Fire statistics indicators */}
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      <div className="bg-slate-900/50 border border-white/5 p-3 rounded-xl flex items-center justify-between">
                        <div>
                          <span className="text-[9px] uppercase font-bold text-gray-500 block">Severity</span>
                          <strong className="text-xs text-red-400 block mt-0.5">{selectedHistoryItem.fire_severity}</strong>
                        </div>
                        <Flame size={18} className="text-red-400/40" />
                      </div>
                      <div className="bg-slate-900/50 border border-white/5 p-3 rounded-xl flex items-center justify-between">
                        <div>
                          <span className="text-[9px] uppercase font-bold text-gray-500 block">Water Consumption</span>
                          <strong className="text-xs text-blue-400 block mt-0.5">{selectedHistoryItem.water_consumption} L</strong>
                        </div>
                        <Droplet size={18} className="text-blue-400/40" />
                      </div>
                      <div className="bg-slate-900/50 border border-white/5 p-3 rounded-xl flex items-center justify-between">
                        <div>
                          <span className="text-[9px] uppercase font-bold text-gray-500 block">Time to Control</span>
                          <strong className="text-xs text-white block mt-0.5">{selectedHistoryItem.time_to_control}</strong>
                        </div>
                        <Clock size={18} className="text-gray-500" />
                      </div>
                      <div className="bg-slate-900/50 border border-white/5 p-3 rounded-xl flex items-center justify-between">
                        <div>
                          <span className="text-[9px] uppercase font-bold text-gray-500 block">Time to Extinguish</span>
                          <strong className="text-xs text-white block mt-0.5">{selectedHistoryItem.time_to_extinguish}</strong>
                        </div>
                        <Clock size={18} className="text-gray-500" />
                      </div>
                    </div>
                    ...
                    <div className="bg-blue-500/5 border border-blue-500/10 p-4.5 rounded-xl space-y-1">
                      <span className="text-[10px] uppercase font-bold text-blue-400 block tracking-wider">Fire Incident resolution summary</span>
            ...
            <div className="lg:col-span-5 glass-panel p-6 rounded-2xl flex flex-col justify-between">
              ...
              <div className="my-6 p-4 rounded-xl bg-slate-900/50 border border-white/5 space-y-3.5 text-xs text-gray-300 shadow-inner leading-relaxed">
                ...
                <div className="flex gap-2">
                  ...
                  <p><strong>Priority Overrides:</strong> Traffic signals are automatically requested for override when engines enter the 500m threshold along this route.</p>
                </div>
              </div>

              <div className="text-[10px] text-gray-500 border-t border-white/5 pt-4 text-center">
                Fire station operators have write permissions to alter details to align with physical changes.
              </div>
            </div>
          </div>
        )}
          ...
          <div className="modal-card glow-red">
            ...
            <form onSubmit={handleResolveSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] uppercase font-bold tracking-wider text-gray-400 mb-1.5">Fire Severity</label>
                  <select
                    value={resolveSeverity}
                    onChange={(e) => setResolveSeverity(e.target.value)}
                    className="w-full px-3 py-2 rounded-xl glass-input text-white text-xs focus:border-blue-500 shadow-inner"
                  >
                ...
                <div>
                  <label className="block text-[10px] uppercase font-bold tracking-wider text-gray-400 mb-1.5">Water Consumption (Liters)</label>
                  <input
                    type="number"
                    required
                    placeholder="e.g. 1500"
                    value={resolveWater}
                    onChange={(e) => setResolveWater(e.target.value)}
                    className="w-full px-3 py-2 rounded-xl glass-input text-white text-xs focus:border-blue-500 shadow-inner"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] uppercase font-bold tracking-wider text-gray-400 mb-1.5">Time to Control (Mins)</label>
                  <input
                    type="number"
                    required
                    placeholder="e.g. 15"
                    value={resolveTimeToControl}
                    onChange={(e) => setResolveTimeToControl(e.target.value)}
                    className="w-full px-3 py-2 rounded-xl glass-input text-white text-xs focus:border-blue-500 shadow-inner"
                  />
                </div>

                <div>
                  <label className="block text-[10px] uppercase font-bold tracking-wider text-gray-400 mb-1.5">Time to Extinguish (Mins)</label>
                  <input
                    type="number"
                    required
                    placeholder="e.g. 35"
                    value={resolveTimeToExtinguish}
                    onChange={(e) => setResolveTimeToExtinguish(e.target.value)}
                    className="w-full px-3 py-2 rounded-xl glass-input text-white text-xs focus:border-blue-500 shadow-inner"
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block text-[10px] uppercase font-bold tracking-wider text-gray-400 mb-1.5">Incident Resolution Summary</label>
                <textarea
                  required
                  rows="2"
                  placeholder="e.g. Gas leak plugged and area ventilated, Fire successfully extinguished at second level..."
              ...
              <div className="flex gap-3 justify-end pt-2">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 rounded-xl text-xs font-bold text-white transition shadow shadow-red-900/30"
                >
                  Confirm Resolution
                </button>
              </div>
