...
import { ShieldAlert, Flame, Send, CheckCircle2, Play, MapPin, Clock, Navigation, Power } from 'lucide-react';
...
export default function FireStationDashboard({ user, onLogout }) {
  ...
  useEffect(() => {
    ...
    socket.on('vehicle_tracking_update', (data) => {
      console.log('Fire station socket: vehicle tracking tick:', data);
  ...
  const mapService = mapDispatch ? {
    ...
    type: mapDispatch.service_type || 'fire_station',
    ...
    <div className="min-h-screen bg-[#070b13] flex flex-col">
      ...
      <nav className="glass-panel border-b border-white/5 py-4 px-6 flex justify-between items-center shadow-lg relative z-20">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-red-600/10 text-red-400 rounded-lg border border-red-500/20">
            <Flame size={20} />
          </div>
          <div>
            <h1 className="text-lg font-bold text-white leading-tight">{user.name}</h1>
            <p className="text-xs text-gray-400">Fire Station Emergency Dispatch Console</p>
          </div>
        </div>
              ...
              {activeDispatch ? (
                <div className="space-y-4">
                  <div className="bg-slate-900/50 p-4 rounded-xl border border-white/5 space-y-2">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] uppercase font-bold text-red-400">Incident Type</span>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3 pt-2">
                    <div>
                      <label className="block text-[10px] font-bold uppercase text-gray-400 mb-1.5">Select Fire Engine</label>
                      <select
                    ...
                    <button
                      ...
                      className="w-full py-3 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 transition text-xs font-bold text-white flex items-center justify-center gap-1.5 shadow-lg"
                    >
